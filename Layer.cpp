...
Layer::Layer(int prev_layer_size, int num_neurons){
    ...
    this->layerOutputs.resize(num_neurons);
...
std::vector<double> Layer::getLayerOutputs(){
    //std::vector<double> layerOutputs(this->neurons.size());
...
void Layer::printLayerOutputs(){
    ...
    for(int i=0 ; i< this->layerOutputs.size(); i++){
...
{
    for(int i=0; i < this->neurons.size(); i++){
        this->neurons[i].activate(prev_layer_values);
    }
}
